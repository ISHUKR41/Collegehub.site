/**
 * Hero.tsx - Landing page hero section with Framer + GSAP hybrid motion.
 *
 * GSAP is used only for continuous decorative orb movement so the main
 * content animation remains lightweight and smooth.
 */
...
import { ArrowRight, BookOpen, Code2, Sparkles, Star, Users, Layers } from 'lucide-react';
...
export default function Hero() {
  ...
  useEffect(() => {
    if (!orbPrimaryRef.current || !orbSecondaryRef.current) {
      return;
    }
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.to(orbSecondaryRef.current, {
        x: -32,
        y: 26,
        duration: 9,
        repeat: -1,
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden" aria-label="Hero section">
      <div className="absolute inset-0">
        ...
        <div
          ref={orbPrimaryRef}
          className="absolute top-1/4 left-1/4 w-[500px] h-[500px] rounded-full bg-[#6366f1]/[0.07] blur-[120px]"
        />
        <div
          ref={orbSecondaryRef}
          className="absolute bottom-1/4 right-1/4 w-[400px] h-[400px] rounded-full bg-[#8b5cf6]/[0.05] blur-[100px]"
        />
      ...
      <div className="relative z-10 container-custom text-center px-4">
        ...
        >
          Structured learning paths for Class 9-10 CBSE and programming tracks.
          Track progress, detect weak topics, and resume exactly where you left off.
        </motion.p>
        ...
        >
          {[
            ...
            { icon: Layers, text: '50+ Structured Courses' },
